...
if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error(
        '❌ 缺少必要的环境变量，请确保.env.local文件中包含 NEXT_PUBLIC_SUPABASE_URL 和 SUPABASE_SERVICE_ROLE_KEY',
    );
    process.exit(1);
}
    ...
    try {
        for (const fund of initialFundsData) {
            try {
                ...
                const { error } = await supabase.from('funds').insert([fund]);
        ...
        return {
            ...
            successCount,
            totalCount: initialFundsData.length,
        };
    } catch (error) {
        console.error(
            '插入数据过程中发生错误:',
